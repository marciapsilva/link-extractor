...
describe('getLinksFromMd()', () => {
  ...
  it('Deveria retornar array com os links para uma string com MAIS DE UM link', () => {
    let message = `Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor  incididunt ut [labore](https://en.wiktionary.org/wiki/labore) et [dolore](https://en.wiktionary.org/wiki/dolore) magna aliqua. Ut enim ad minim veniam, 
  ...
  it('Deveria retornar "Erro: parâmetro não informado"', () => {
    expect(getLinksFromMd()).to.equal('Erro: parâmetro não informado');
  });
  it('Deveria retornar "Erro: parâmetro não é uma string" para parâmetro que for número', () => {
    expect(getLinksFromMd(436437)).to.equal('Erro: parâmetro não é uma string');
  });
});